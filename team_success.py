...
# Модель оценки
# Описание логики:
# - Начальный score = 50, повышается за положительные события, понижается за негативные
# - Критичные ошибки (нет разработчиков, аналитиков при высоких ожиданиях, отсутствие QA и т.д.) ведут к немедленному провалу
# - synergy_score учитывает инициативу и фидбек, disruption_score — конфликты, игнор, уход и т.д.
# - Дополнительные комментарии объясняют риски (один разработчик, нет QA, низкая мотивация и т.д.)
# - Результат выбирается из 6 классов: Успешный MVP, Частичный MVP, Фича с багами, Маленький результат, Сырой прототип, Провал
...
def success_level(team, events, expectation, deadline):
    ...
    if ba_count == 0 and expectation not in ["Концепт", "Фича"]:
    ...
    if flegma_count > len(team) / 2 and low_motivation_count > len(team) / 2:
        disruption_score += 1
    if deadline == "1 месяц" and expectation == "Готовый продукт":
        return "Провал", 10, ["Слишком короткий срок для полноценного продукта."]
    ...
    if role_counter['Product Owner'] > 1 or role_counter['Project Manager'] > 1:
    ...
    elif dev_count >= 2 and qa_count > 0 and expectation == "Фича" and disruption_score <= 1:
        return "Фича с багами", max(score - 5, 40), comments
    elif dev_count >= 1 and qa_count >= 1 and expectation == "Фича":
        return "Маленький результат", max(score - 15, 30), comments
